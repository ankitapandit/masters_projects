...
def parse_args():
    ...
    parser.add_argument("--img_path", type=str, default="", help="path to the image used for edge detection")
    parser.add_argument("--kernel", type=str, default="sobel", choices=["prewitt", "sobel", "Prewitt", "Sobel"], help="type of edge detector used for edge detection")
    parser.add_argument("--result_saving_directory", dest="rs_directory", type=str, default="./results/", help="directory to which results are saved (do not change this arg)")
...
def normalize(img):
    ...
    for row_val in img:
        value_norm = list()
        for pixel_val in row_val:
            pixel_norm = rgb * ((pixel_val - img_min)/(img_max - img_min))
            value_norm.append(pixel_norm)
        img_norm.append(value_norm)

    img_min = np.amin(img_norm)
    img_max = np.amax(img_norm)
    img = img_norm
...
def edge_magnitude(edge_x, edge_y):
    ...
    row_zero = len(edge_x)
    pixel_zero = len(edge_y[0])
    width = pixel_zero
    height = row_zero
    dim = (height, width)

    edge_mag = np.zeros(dim)
    for i in range(0, row_zero):
        for j in range(0, pixel_zero):
            edge_mag[i][j] = np.sqrt(edge_x[i][j]**2 + edge_y[i][j]**2)
    edge_mag = np.asarray(edge_mag)
